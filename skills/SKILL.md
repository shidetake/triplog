...
主要フィールド（`timezone` は IANA 表記の旅行先 TZ。Sony 銀行メール単独取引の `occurredAt` を JST→現地に補正するため、新規旅行では**必ず**設定する）：
...
## 3.6. Clover full-receipt enrichment（reuse-raw でも必ず実行）

Clover (`app@clover.com`) のメールはテキスト本文に金額と店名しか出ず、品目は HTML 内 `<!-- FULL TRANSACTION RECEIPT -->` ブロックの SendGrid URL の先（`https://www.clover.com/p/<id>`）にある。`stripHtml` 後の `body` には URL が消えてしまうので、改めて HTML を取り直してから follow する:

1. `from` が `app@clover.com` を含み、かつ `linkedContent` が未設定の raw を対象に絞る
2. `mcp__gmail__read_email` で同じ messageId を再取得（HTML 込みで返る）
3. HTML から「`>full transaction receipt<`」直前の `<a href="https://[a-z0-9]+\.ct\.sendgrid\.net/ls/click?[^"]+"` を抽出
4. WebFetch で SendGrid URL を叩く → `REDIRECT DETECTED` で `https://www.clover.com/p/<id>` が返る
5. その redirect URL を再度 WebFetch。プロンプトは「Clover hosted receipt の line items を `- <name>: $<price>` 形式で1行ずつ返す。subtotal/tax/tip/total/payment は除外」
6. 返ってきたテキストを `RawMessage.linkedContent` にセットして raw JSON にマージ保存

WebFetch が 404 / expired を返したら linkedContent は未設定のまま残し、detail は空欄で続行する（過剰に retry しない）。

reuse-raw モードでも、`linkedContent` が無い Clover 行があればここで埋める（Square と同じ運用）。
...
- 既知ソースは専用パーサで `RawExpense` 化：
  ...
  - `parsers/clover.ts`（app@clover.com、§3.6 で `linkedContent` 埋めた前提で品目入り）
...
- 件数（行数）
- カテゴリ別合計（円）
- 対象シートの現在の行範囲（`B19:J80` など、読み取りのみ）
- 上書きされる行範囲
...
- 範囲： `<sheetName>!B19:F<最終行>` と `<sheetName>!H19:J<最終行>` の2範囲（**G列は触らない**）
- ツール: `mcp__gsheets__batch_update_cells`（必ず batch、単一の `update_cells` で B:J を一括書きしないこと — G の formula を破壊する）
- I列（計算対象外）は明示的に `FALSE`
- J列（備考）は `RawExpense.notes` をそのまま書く。空でも書き出す（既存の備考を上書き）
- 数式は使わず値で書く
...
- detail 列(E) にチップ・タックス・「部屋付け」等のメタ情報を入れる（情報量0。品目のみ書く）。メタは J 列（備考 / `notes`）へ
