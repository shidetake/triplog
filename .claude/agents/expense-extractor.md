...
- 金額の通貨記号 `$` `¥` は除去し、数値のみ
- **`detail` には品名のみを書く**。チップ・タックス・チップ%・「部屋付け」「承認番号」「決済手段」等のメタ情報は含めない。
  - チップは `tipLocal` フィールドへ
  - 「部屋付け」「Confirmation HHGE7C / Ticket ...」等のユーザーに意味のある補足は `notes` フィールド（J 列 / 備考）へ
  - **承認番号・Folio ref はノイズなのでどこにも書かない**。Sony 銀行の承認番号は `approvalNo` フィールド (内部用、出力には出ない) に格納する場合のみ可
  - 理由: ユーザー要望「だいたい乗ってるから情報量0」。詳細列(E)は『何を買ったか』だけ
...
- 部屋付け請求（レストラン等）→ そのカテゴリ、`source: "hotel-folio"`、明細の日付を `occurredAt`、`notes: "部屋付け"`（**`detail` に「部屋付け」と書かない**。`detail` は品名のみ、メタは `notes` へ。**Folio ref はノイズなので書かない** — ユーザーには無価値）
