...
- **`detail` には品名のみを書く**。チップ・タックス・チップ%・「部屋付け」「承認番号」「決済手段」等のメタ情報は含めない。
  ...
  - 「部屋付け」「承認番号:XXXX」「Confirmation HHGE7C / Ticket ...」等の決済参照は `notes` フィールド（J 列 / 備考）へ
...
- 部屋付け請求（レストラン等）→ そのカテゴリ、`source: "hotel-folio"`、明細の日付を `occurredAt`、`notes: "部屋付け"`（**`detail` に「部屋付け」と書かない**。`detail` は品名のみ、メタは `notes` へ。Folio ref 等の参照番号があれば `notes: "部屋付け (Folio ref XXXXX)"`）
